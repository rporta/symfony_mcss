...
class collapsible extends createClass
{
	public $id;
	public $type;
	public $html;
	...
	public function __construct($arg = NULL){
		...
		$this->id = 'collapsible-'.$this->createID(5);
	...
	public function refreshInfo(){
		...
		$tempHtml = '<div id="{ID}" class="collapsibles {TEXTCOLOR} {BACKGROUNDCOLOR} {CONTAINER} {VALIGN} {TEXTALING} {FLOAT} {SHADOW} {TRUNCATE} {CARDPANEL} {HOVERABLE}">{obj:html}</div>';
		...
		if($arg == 'html'){
			if(!empty($this->obj)){
				foreach ($this->obj as $key => $idObj) {
					$backgroundColor =  $this->backgroundColors($idObj->backgroundColor);

 					if ($key % 2 == 0){
					    $sub[] = "<div class='collapsible-body {$backgroundColor}'>".$idObj->html.'</div>';
					    $objHtml[] = "<li>".implode("", $sub)."</li>";
					    unset($sub, $backgroundColor);
					}else{
					    $sub[] = '<div class="collapsible-header {$backgroundColor}">'.$idObj->html.'</div>';
					}
				}
